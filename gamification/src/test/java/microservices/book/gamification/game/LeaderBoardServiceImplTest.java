...
public class LeaderBoardServiceImplTest {
    ...
    public void retrieveLeaderBoardTest() {
        // given
        LeaderBoardRow scoreRow = new LeaderBoardRow(UUID.randomUUID().toString(), 300L, List.of());
        given(scoreRepository.findFirst10()).willReturn(List.of(scoreRow));
        given(badgeRepository.findByUserIdOrderByBadgeTimestampDesc(UUID.randomUUID().toString()))
                .willReturn(List.of(new BadgeCard(UUID.randomUUID().toString(), BadgeType.LUCKY_NUMBER)));
        ...
        List<LeaderBoardRow> expectedLeaderBoard = List.of(
                new LeaderBoardRow(UUID.randomUUID().toString(), 300L,
